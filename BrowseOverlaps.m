function [ output_args ] = BrowseOverlaps(traces,moviefile,NeuronID )
close all;
...
plot(zscore(traces(NeuronID,:)));hold on;plot(FT(NeuronID,:)*5);axis tight;

for i = 1:length(buddies)
    a(i+1) = subplot(length(buddies)+1,1,i+1);
    plot(zscore(traces(buddies(i),:)));hold on;plot(FT(buddies(i),:)*5);title(int2str(buddies(i)));axis tight;
end
 linkaxes(a,'x');
