function [ output_args ] = FL_Placefields2(GoodSignalTrace,GoodICf,xOut,yOut)
...
for i = 1:NumCells
  ...
  %figure;plot(GoodSignalTrace(i,:));hold off;pause;
  if(sum(GoodSignalTrace(i,:)) > 0)
    
    spiketrain(i,:) = DWS_oopsi(f,V,P);display('ran oops');
...
for i = 1:NumCells
    ...
    % find 4 maxima
    [max1,idx1] = nanmax(GoodSignalTrace(i,1:ceil(flen/4)));
    [max2,idx2] = nanmax(GoodSignalTrace(i,ceil(flen/4)+1:ceil(flen/2)));idx2 = idx2+ceil(flen/4); 
    [max3,idx3] = nanmax(GoodSignalTrace(i,ceil(flen/2)+1:ceil(flen/4*3)));idx3 = idx3+ceil(flen/2);
    [max4,idx4] = nanmax(GoodSignalTrace(i,ceil(flen*3/4)+1:end));idx4 = idx4+ceil(flen*3/4);
