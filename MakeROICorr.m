function [varargout] = MakeROICorr(varargin)
...
for i = 1:NumInputs
  ...
  CorrP{i} = zeros(length(varargin{i}),NumFrames,'single');
  CorrR{i} = zeros(length(varargin{i}),NumFrames,'single');
  disp(['making correlation traces for ',int2str(length(varargin{i})),' ROIs']);
end
    ...
    for j = 1:size(FrameChunk,3)
        ...
        for k = 1:NumInputs
            for m = 1:length(varargin{k})
                ...
                [CorrR{k}(m,FrameList(j)),CorrP{k}(m,FrameList(j))] = corr(frame(varargin{k}{m}), 
            end
        end
    end
    p.progress;    
