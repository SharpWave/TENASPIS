...
MaxSignalRad = 5; % maximum radius from middle of ICA for main component

OutNoiseRad = 40; % Outer circle radius of noise ring, big enough to form a complete circle around the signal
InNoiseRad = 6; % Inner circle radius of noise ring, we want some overlap with the signal
...
for i = 1:NumICA
   All_Mask = (All_Mask+CellIC{i}) > 0;
   Sum_Mask = Sum_Mask+CellIC{i};
end
figure(FigNum);imagesc((1:Xdim)*MicronsPerPix,(1:Ydim)*MicronsPerPix,Sum_Mask);
FigNum = FigNum + 1;
...
save SignalTracev7.mat GoodSignalTrace GoodCom GoodICf ThreshIC GoodIC NoiseMask COM d scriptname filestruct;
