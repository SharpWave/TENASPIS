function Set_T_Params2(moviefile)
...
T_PARAMS.LowPassRadius = 3; % Smoothitng radius for low pass disk-kernel filtering. EDIT:SPACE

%% ExtractBlobs / SegmentFrame params
T_PARAMS.threshold = 0.001; % Pixel intensity baseline threshold for detecting blobs. Lower means more blobs but more noise and longer runs
...
T_PARAMS.MinSolidity = 0.6; % Minimum blob 'solidity', which is the ratio of the perimeter of the convex hull to the actual perimeter. 
...
T_PARAMS.MaxCentroidTravelDistance = 8; % maximum net distance that the centroid of a transient can travel. 
                                        % Eliminates spurious blobs from overlapping transients.
                                        % EDIT:SPACE
                                        
T_PARAMS.MinNumFrames = 1; % minimum number of frames for transient to be included. EDIT:TIME
...
T_PARAMS.SmoothSize = 2;                     % length of window for temporal smoothing of traces.  EDIT:SPACE
...
T_PARAMS.MinPSALen = 2;     % minimum duration of PSA epochs, enforced right after detection. Helps to eliminate noise; 250ms is awfully short for a spiking epoch
                            % EDIT:TIME
...
T_PARAMS.MaxGapFillLen = 2; % After detecting rising slopes, if the gaps between PSA epochs are this # of samples or smaller, fill them in.
                            % smooths the skippyness in some borderline
                            % cases. % EDIT:TIME 
                            
%% FinalizeData
T_PARAMS.MinNumPSAepochs = 1; % minimum number of PSA epochs for inclusion in final ROI set. i.e., we delete the ones with less than this
